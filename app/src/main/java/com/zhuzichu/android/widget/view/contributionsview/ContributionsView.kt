...
class ContributionsView @JvmOverloads constructor(
    ...
    var isMondayStart: Boolean = true
        set(value) {
            field = value
            invalidate()
        }
    ...
    init {
        ...
        squareVerticalSpace =
            xmlAttributes.getDimension(R.styleable.ContributionsView_square_vertical_space, 4 * dpi)
        squareHorizontalSpace =
            xmlAttributes.getDimension(R.styleable.ContributionsView_square_vertical_space, 4 * dpi)
        ...
        contributionsLeftSpace = xmlAttributes.getDimension(
            R.styleable.ContributionsView_contributions_left_space,
            4 * dpi
        )
        contributionsTopSpace = xmlAttributes.getDimension(
            R.styleable.ContributionsView_contributions_top_space,
            7 * dpi
        )

        isMondayStart =
            xmlAttributes.getBoolean(R.styleable.ContributionsView_is_monday_start, true)
        ...
        dayOfWeekPaint = DayOfWeekPaint(
            xmlAttributes.getDimension(
                R.styleable.ContributionsView_day_of_week_font_size,
                15 * dpi
            ),
            xmlAttributes.getColor(
                R.styleable.ContributionsView_day_of_week_font_color,
                Color.GRAY
            ),
            dayOfWeeks = *defaultDayOfWeeks
        )
        isDisplayDayOfWeek =
            xmlAttributes.getBoolean(R.styleable.ContributionsView_is_display_day_of_week, true)
        ...
        isDisplayMonth =
            xmlAttributes.getBoolean(R.styleable.ContributionsView_is_display_month, true)
        ...
        legendTextSpace =
            xmlAttributes.getDimension(R.styleable.ContributionsView_legend_text_space, 7 * dpi)
        legendSquareSpace =
            xmlAttributes.getDimension(R.styleable.ContributionsView_legend_square_space, 4 * dpi)
        legendSquareSize =
            xmlAttributes.getDimension(R.styleable.ContributionsView_legend_square_size, 15 * dpi)
        legendTopSpace =
            xmlAttributes.getDimension(R.styleable.ContributionsView_legend_top_space, 7 * dpi)
        isDisplayLegend =
            xmlAttributes.getBoolean(R.styleable.ContributionsView_is_display_legend, true)
    ...
    fun setContributionsDateMap(contributions: HashMap<Date, Int>) {
        this.contributions = contributions.mapKeysTo(HashMap()) {
            DateTimeUtils.toZonedDateTime(Calendar.getInstance().apply { time = it.key })
                .toLocalDate()
        }
        invalidate()
    }
    ...
    fun setCommit(date: Date, amount: Int) {
        val localDate = DateTimeUtils.toZonedDateTime(Calendar.getInstance().apply { time = date })
            .toLocalDate()
        contributions[localDate] = amount
        invalidate()
    }
    ...
    fun setEndDate(date: Date) {
        endLocalDate = DateTimeUtils.toZonedDateTime(Calendar.getInstance().apply { time = date })
            .toLocalDate()
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val minWidth = width
        val minHeight = paddingTop + if (isDisplayMonth) {
            monthPaint.getTextHeight() + contributionsTopSpace
        } else {
            0f
        } +
                squareSize * 7 + squareVerticalSpace * 6 +
                if (isDisplayLegend) {
                    legendTopSpace + if (legendPaint.getTextHeight() > legendSquareSize) {
                        legendPaint.getHeight()
                    } else {
                        legendSquareSize
                    }
                } else {
        ...
        setMeasuredDimension(
            ...
            resolveSize(minHeight.toInt(), heightMeasureSpec)
        )
    }

    override fun onDraw(canvas: Canvas) {
        val offsetStart =
            (width - dayOfWeekPaint.getDayOfWeekWidth() - paddingStart - paddingEnd - contributionsLeftSpace) % (squareSize + squareHorizontalSpace) / 2
        if (isDisplayDayOfWeek) {
            drawDayOfWeek(
                canvas,
                paddingStart.toFloat(),
                paddingTop + if (isDisplayMonth) {
                    monthPaint.getTextHeight()
                } else {
                    0f
                }
            )
        ...
        drawContributions(
            canvas,
            paddingStart + offsetStart + if (isDisplayDayOfWeek) {
                dayOfWeekPaint.getDayOfWeekWidth() + contributionsLeftSpace
            } else 0f,
            paddingTop + if (isDisplayMonth) {
                monthPaint.getTextHeight() + contributionsTopSpace
            } else {
        ...
        if (isDisplayLegend) {
            drawLegend(
                ...
                paddingTop + if (isDisplayMonth) {
                    ...
                    0f
                } + squareSize * 7 + squareVerticalSpace * 6 + legendTopSpace,
                paddingEnd.toFloat()
            )
        }
    }
    ...
    private fun drawDayOfWeek(canvas: Canvas, spaceLeft: Float = 0f, spaceTop: Float = 0f) {
        val dayOfWeeks: List<DayOfWeek> = if (isMondayStart) {
            ...
            arrayListOf(
                arrayListOf(DayOfWeek.SUNDAY),
                DayOfWeek.values().dropLast(1)
            ).flatten()
        }
    ...
    private fun drawLegend(canvas: Canvas, spaceTop: Float = 0f, spaceRight: Float = 0f) {
        val legendAreaWidth =
            legendSquareSize * 5 + legendSquareSpace * 4 + legendPaint.getLessWidth() + legendPaint.getMoreWidth() + legendTextSpace * 2
        var x = canvas.width - spaceRight - legendAreaWidth
        val diff = abs(legendPaint.getTextHeight() - legendSquareSize)
        ...
        evaluations.toSortedMap(kotlin.Comparator { o1, o2 -> o1.ordinal - o2.ordinal })
            .map {
                canvas.drawRect(
                    ...
                    x + legendSquareSize,
                    squareY2,
                    squarePaint.getPaint(it.key)
                )
                x += legendSquareSize + if (it.key == Evaluation.A) legendTextSpace else legendSquareSpace
            }
    ...
    private fun drawContributions(
        ...
        val weeks =
            ((canvas.width - spaceLeft - spaceRight) / (squareSize + squareHorizontalSpace)).toInt()
        var x1 = spaceLeft
        ...
        for (week in 1..weeks) {
            ...
            for (n in 1..7) {
                ...
                // 月の描画
                if (isDisplayMonth && commitDate.dayOfMonth == 1) drawMonth(
                    canvas,
                    commitDate.month,
                    x1,
                    paddingTop.toFloat()
    ...
    class SavedState : BaseSavedState {
        ...
        internal constructor(source: Parcel) : super(source) {
            ...
            contributions =
                source.readHashMap(ClassLoader.getSystemClassLoader()).toCast()
            evaluations =
                source.readHashMap(ClassLoader.getSystemClassLoader()).toCast()
        }
